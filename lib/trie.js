 ...
 *   When created, a Trie creates an empty root node with empty children array.
 *  
 */
class Trie {
    constructor() {
    ...
    addWord(word) {
        ...
        for (level = 0; level < length; level++) {
            index = Constant.alphabet.indexOf(word[level]);
    ...
    */
    suggestionsRec(root, currPrefix, suggestions) {
        ...
        // All children node pointers are empty
        if (root.isLastNode())
            return;

        for (let i = 0; i < Constant.alphabetSize; i++) {
            if (root.children[i] !== undefined) {
                // append current character to currPrefix string
                currPrefix = currPrefix.concat(Constant.alphabet[i]);
    ...
    */
    filterSuggestions(suggestions, query, limit) {
        ...
        suggestions.forEach((suggestion) => {
            ...
            let suggestionLength = suggestion.length;           
            let distance = suggestionLength - wordLength;
            //Convert the levenshtein distance to the percentage of string similarity
            let similarityPercentage = ((suggestionLength - distance) / suggestionLength) * 100;
    ...
    */
    getAutoSuggestions(root, query) {
        ...
        for (level = 0; level < n; level++) {
            ...
            let index = Constant.alphabet.indexOf(query[level]);
